...
DINO_CHECKPOINT_PATH = 'your_dino_checkpoint_path' # ! Please specify the checkpoint path for the DINO backbone.
PointMAE_CHECKPOINT_PATH = 'your_point_mae_checkpoint_path' # ! Please specify the checkpoint path for the PointMAE backbone.
...
class FeatureExtractors(torch.nn.Module):
    def __init__(self, device, 
        ...
        ## RGB backbone
        self.rgb_backbone = timm.create_model(model_name = rgb_backbone_name, pretrained = False, checkpoint_path=DINO_CHECKPOINT_PATH,**kwargs)
        ...
        self.xyz_backbone.load_model_from_ckpt(PointMAE_CHECKPOINT_PATH) # ! Please specify the checkpoint path for the PointTransformer backbone.
        # ! Use only the first k blocks.
